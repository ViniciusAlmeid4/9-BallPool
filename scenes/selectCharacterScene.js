...
function updateCharacterCardVisual(cardData) {
    ...
    if (!cardData || !cardData.uiElements) {
        console.warn("[updateCharacterCardVisual] cardData ou uiElements faltando para:", cardData ? cardData.id : "ID Desconhecido");    
        return;
    }
    ...
    if (!this.charactersSelectedBy) {
        console.error("[updateCharacterCardVisual] ERRO: this.charactersSelectedBy é undefined! Contexto 'this':", this);
        return;
    }
...
function handleCardClick(characterId) {
    if (!this.charactersDataList) {
        console.error("[handleCardClick] ERRO: this.charactersDataList é undefined.");
        return; 
    }
    const characterData = this.charactersDataList.find(c => c.id === characterId);

    if (!characterData) {
        console.warn("[handleCardClick] characterData não encontrado para id:", characterId);
        return;
    }
    if (!this.charactersSelectedBy) {
        console.error("[handleCardClick] ERRO: this.charactersSelectedBy é undefined.");
        return;
    }

    if (this.playerTurn === 1 && this.charactersSelectedBy.p2 === characterId) return;
    if (this.playerTurn === 2 && this.charactersSelectedBy.p1 === characterId) return;

    SoundManager.playSfx('clickSfx');

    if (this.playerTurn === 1) {
        if (this.charactersSelectedBy.p1) {
            const prevCharDataP1 = this.charactersDataList.find(c => c.id === this.charactersSelectedBy.p1);
            this.charactersSelectedBy.p1 = null;
            if (prevCharDataP1) updateCharacterCardVisual.call(this, prevCharDataP1);
        }
        this.charactersSelectedBy.p1 = characterId;
        this.playerTurn = 2;
    } else { 
        if (this.charactersSelectedBy.p2) {
            const prevCharDataP2 = this.charactersDataList.find(c => c.id === this.charactersSelectedBy.p2);
            this.charactersSelectedBy.p2 = null;
            if (prevCharDataP2) updateCharacterCardVisual.call(this, prevCharDataP2);
        }
...
function create() {
    ...
    sceneContext.add.image(screenCenterX, 80, 'titleSelectChar').setOrigin(0.5).setScale(0.7);
    ...
    for (let i = 0; i < numChars; i++) {
        cardPositions.push({ x: startX + i * (200 + 50), y: screenCenterY - 0 }); // 200 card, 50 espaço
    }
    ...
    sceneContext.charactersDataList.forEach((charData, index) => {
        if (index >= cardPositions.length) {
            console.warn("Mais personagens definidos em charactersDataList do que posições em cardPositions.");
            return;
        }
        ...
        const frameWhiteImg = sceneContext.add.image(0, 0, 'frameWhite').setOrigin(0.5).setScale(cardScale);
        const frameYellowImg = sceneContext.add.image(0, 0, 'frameYellow').setOrigin(0.5).setScale(cardScale).setVisible(false);
        const portraitImg = sceneContext.add.image(0, 10, charData.portraitKey).setOrigin(0.5).setScale(cardScale * 1   );
        const playerTextIndicator = sceneContext.add.image(0, - (frameWhiteImg.displayHeight * cardScale * 0.5) - 30, 'player1TextImg').setOrigin(0.5, 1).setScale(0.6).setVisible(false);

        const nameImageYOffSet = (frameWhiteImg.displayHeight * cardScale * 0.5) + 30;
        const nameImage = sceneContext.add.image(0, nameImageYOffSet, charData.nameImageKey).setOrigin(0.5, 0).setScale(0.6);

        cardContainer.add([frameWhiteImg, frameYellowImg, portraitImg, playerTextIndicator, nameImage]);
        ...
        cardContainer.on('pointerover', () => { 
            if (!charData.uiElements.frameYellow.visible) cardContainer.setScale(baseContainerScale * 1.05);
        });
    ...
    sceneContext.startButton = sceneContext.add.image(screenCenterX, sceneContext.cameras.main.height - 70, 'startButtonImg')
        .setOrigin(0.5)
        .setScale(0.7)
    ...
    sceneContext.startButton.on('pointerdown', () => {
        if (sceneContext.charactersSelectedBy.p1 && sceneContext.charactersSelectedBy.p2) {
            SoundManager.playSfx('clickSfx');

            const char1Data = sceneContext.charactersDataList.find(c => c.id === sceneContext.charactersSelectedBy.p1);
            const char2Data = sceneContext.charactersDataList.find(c => c.id === sceneContext.charactersSelectedBy.p2);

            if (char1Data && char2Data) {
                player1.character = char1Data.constructorFunc();
                player2.character = char2Data.constructorFunc();
                
                console.log("P1 char:", player1.character.charName, "P2 char:", player2.character.charName);
                SoundManager.stopMusic();
                sceneContext.scene.start('MainScene');
            } else {
                console.error("Não foi possível encontrar os dados dos personagens selecionados para iniciar o jogo.");
            }
        }
    });
}
